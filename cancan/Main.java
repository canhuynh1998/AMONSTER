...
public class Main {
    ...
    */
    ...
    private static void fighting(Player player1, Monster[] fat) {
        ...
        do {
            ...
            if (monsterChoice > champChoice) {
                ...
                healthDecrease(player1.getChamp().getName(), fat[level].getPhysicalRate(),
                        player1.getChamp().getHealthCap());
            } else if (monsterChoice < champChoice) {
                fat[level].setHealthDecreasse(player1.getChamp().getPhysicalRate());

                healthDecrease(fat[level].getName(), player1.getChamp().getPhysicalRate(),
                        fat[level].getHealthCap());
            } else {
    ...
    /*
    healthDecrease(String, float, float) function
    Display the character health loss
     */
    private static void healthDecrease(String healthLossCharacter, float PhysicalRate, float HealthCap) {
        System.out.println(healthLossCharacter + " health's is decreased by " + PhysicalRate);
        System.out.println(healthLossCharacter + " health's is " + HealthCap);
    }
